...
lookup. This module only adds what is specific to buying a SHIP:
  ...
  * a belt-and-braces `_sells_ship` guard over each station's `ships` list, and
...
from ..search.spansh import (Http, NavError, STATIONS_URL, _DEFAULT_UA, distance_sort,
...
def _nearest_match(results: list[dict], resolved, pad_size: str | None) -> dict | None:
    ...
    and has a big-enough pad. Pad is already filtered server-side; `_pad_ok` is a backstop and
    fleet carriers are dropped as transient (they jump)."""
    for r in results:
...
def _to_result(r: dict, ship_name: str) -> ClosestResult:
    extra = {
...
def find_closest_ship(resolved, current_system: str, http: Http, *,
    ...
    if not results:
        ...
        # "none with a big-enough pad" rather than "no shipyard sells it".
        if pad_size:
    ...
    return _to_result(match, resolved.name)
