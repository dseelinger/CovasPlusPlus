...
- [ ] **Look-to-place:** turn to face a spot, say *"pin the HUD here"* → the panel **swings to your gaze**, centred, keeping distance/width. Turn elsewhere and pin again to confirm it follows your heading. *"Reset the HUD position"* returns it to straight-ahead defaults. (With the VR overlay **off**, *"pin the HUD here"* replies that it isn't running — no crash.)
- [ ] **Look-to-place captures pitch too (#107):** look **down** at the dash, *"pin the HUD here"* → the panel **drops to your gaze** and its top **leans toward you** so it reads head-on (not edge-on). If it leans *away*, the `pitch_deg` sign is flipped. Look **up** and pin → it rises and tilts the other way. Looking level is unchanged from before. Look nearly straight down and pin → it **clamps** (~60° tilt, ≤2 m down), no glitch.
