...
- [ ] **Reply language (issue #182):** *"COVAS, reply in German."* → confirms; 📋 `overrides.json` gets `language.reply = "German"`. The **next reply is spoken and shown in German**, while ED proper nouns (system/station/ship names) stay in their original form. Say *"reply in English"* to switch back → replies return to English. (For a full German experience also pick a German-capable voice — see the note on the [Reply language](docs/using/language.md) page.)
- [ ] **STT follows the reply language (issue #197):** with `[whisper].language` left at its default `"follow"` (do **not** change it), set **reply = German** and use a **multilingual** model (e.g. `small`). Speak a German sentence → it's **transcribed as German** (the log shows Whisper running with `de`), no manual `[whisper].language` edit. Set reply back to English → German speech is no longer forced (transcription tracks `en`).
...
- [ ] **Control-panel UI strings extracted, English unchanged (issue #196):** open the control panel, **Settings**, and each sub-page (**checklist, memory, crew, engineers, macros**) plus the **first-run wizard** — all headings, buttons, nav, placeholders and hints read **exactly as before** (the extraction is English-baseline only; there is no visible change). No raw `{{ t(... }}` text appears anywhere. Because only English ships, setting **reply = German** does **not** translate the panel — it stays fully English (the gate: a language activates only once a complete catalog is contributed, per [translating the UI](docs/using/translating-the-ui.md)); confirm the panel does **not** show a half-German/half-English mix.
