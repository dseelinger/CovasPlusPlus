...
### Backlog
Prompts 1–7, Search 1–6, N1–N11, and **C1–C9** are built and merged. **Outstanding: C10 (voice cast — deterministic identity→voice assignment + Piper/ElevenLabs provider routing) and C11 (drop-in content pipeline — auto-discover local assets + line pools)** in `CLAUDE_CODE_PROMPTS.md`. New work otherwise starts as a new prompt in the pack (one branch per prompt, one fresh session each). **The prompt pack carries the live worklist; this doc carries the architecture.**
