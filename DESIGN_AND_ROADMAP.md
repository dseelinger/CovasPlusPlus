...
### Backlog
**Multi-provider support (issue #10):** additional cloud LLM + TTS providers. TTS track: #14 registry (done) → #15 Edge (done) → #16 OpenAI TTS (done) → #17 Azure Neural (done) → #18 Cartesia. LLM track: #11 provider-agnostic router (foundation) → #12 OpenAI-compatible → #13 Gemini. Otherwise every prompt in `CLAUDE_CODE_PROMPTS.md` (Prompts 1–7, Search 1–6, N1–N11, C1–C11, I1–I9) is built and merged. **The prompt pack / GitHub issues carry the live worklist; this doc carries the architecture.**
