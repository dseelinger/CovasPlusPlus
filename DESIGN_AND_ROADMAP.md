...
### Backlog
**Empty — every prompt in `CLAUDE_CODE_PROMPTS.md` (Prompts 1–7, Search 1–6, N1–N11, C1–C11) is built and merged.** New work starts as a new prompt in the pack (one branch per prompt, one fresh session each). **The prompt pack carries the live worklist; this doc carries the architecture.**
