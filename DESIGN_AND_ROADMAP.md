...
- **Freeze — PyInstaller (one-folder, CPU-only).** Bundles the interpreter + all `requirements.txt` deps into a read-only install tree, so there's no Python prerequisite. One-folder (not one-file) for faster starts and fewer AV false-positives. Spec is `covas.spec`; build deps live in `requirements-build.txt`, not the base runtime.
